...
RE_FINLINE XMVECTOR SplatZero() {
  return XMVectorZero();
}

#if !defined(_XM_NO_INTRINSICS_) && defined(_XM_SSE_INTRINSICS_)

RE_FINLINE XMVECTOR SplatOne(XMVECTOR zero = SplatZero()) {
...
RE_FINLINE XMVECTOR CreateXUnitVector(XMVECTOR one = SplatOne()) {
  return _mm_insert_ps(one, one, 0x0E);
}
RE_FINLINE XMVECTOR CreateYUnitVector(XMVECTOR one = SplatOne()) {
  return _mm_insert_ps(one, one, 0x0D);
}
RE_FINLINE XMVECTOR CreateZUnitVector(XMVECTOR one = SplatOne()) {
  return _mm_insert_ps(one, one, 0x0B);
}
RE_FINLINE XMVECTOR CreateWUnitVector(XMVECTOR one = SplatOne()) {
  return _mm_insert_ps(one, one, 0x07);
}
RE_FINLINE XMVECTOR SetWToZero(FXMVECTOR vec) {
  return _mm_insert_ps(vec, vec, 0x08);
}
RE_FINLINE XMVECTOR SetWToOne(FXMVECTOR vec) {
...
RE_FINLINE XMVECTOR SplatOne() {
  return XMVectorSplatOne();
}
RE_FINLINE XMVECTOR CreateXUnitVector() {
  return g_XMIdentityR0;
}
RE_FINLINE XMVECTOR CreateYUnitVector() {
  return g_XMIdentityR1;
}
RE_FINLINE XMVECTOR CreateZUnitVector() {
  return g_XMIdentityR2;
}
RE_FINLINE XMVECTOR CreateWUnitVector() {
