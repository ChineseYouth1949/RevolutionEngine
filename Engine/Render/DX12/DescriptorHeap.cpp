...
stl::vector<Microsoft::WRL::ComPtr<ID3D12DescriptorHeap>> DescriptorAllocator::sm_DescriptorHeapPool;
...
ID3D12DescriptorHeap* DescriptorAllocator::RequestNewHeap(ID3D12Device* Device, D3D12_DESCRIPTOR_HEAP_TYPE Type) {
  ...
  RE_ASSERT_HR(Device->CreateDescriptorHeap(&Desc, IID_PPV_ARGS(&pHeap)));
...
void DescriptorHeap::Create(ID3D12Device* Device, const std::wstring& Name, D3D12_DESCRIPTOR_HEAP_TYPE Type, uint32_t MaxCount) {
  ...
  RE_ASSERT_HR(Device->CreateDescriptorHeap(&m_HeapDesc, IID_PPV_ARGS(m_Heap.ReleaseAndGetAddressOf())));
...
DescriptorHandle DescriptorHeap::Alloc(uint32_t Count) {
  RE_ASSERT_MSG(HasAvailableSpace(Count), "Descriptor Heap out of space.  Increase heap size.");
